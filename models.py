...
class Item:
    ...
    def create_table(self):
        return self.db.cursor.execute(
            """CREATE TABLE IF NOT EXISTS {table_name}(id integer primary key default {id},
    ...
    async def insert(self, **kwargs):
        try:
            ...
            logger.error("Error saving item: %s", exe)
    ...
    async def show(self):
        res = self.db.cursor.execute(
            "SELECT count(*) FROM {table_name}".format(table_name=self.table_name)
        )
        res = res.fetchall()
        count = sum([i for r in res for i in r])
        logger.info("Total items in %s: %d", self.table_name, count)
...
class User:
    ...
    def create_table(self):
        return self.db.cursor.execute(
            """CREATE TABLE IF NOT EXISTS user(id text primary key default {id},
         created integer default {created}, karma integer default {karma}, about text default {about},
         delay integer default {delay})""".format(
                **self.asdict()
            )
        )
